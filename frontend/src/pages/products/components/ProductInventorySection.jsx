...
export default function ProductInventorySection({ product, isEditing = false, values, errors, onChange }) {
  const stock = Number(isEditing ? values.stockQuantity || 0 : product.stockQuantity || 0);
  ...
  const healthLabel =
    (isEditing ? values.status : product.status) === "out_of_stock" || stock === 0 ? "Out of stock" : product.isLowStock ? "Low stock warning" : "Healthy stock level";
      ...
      <div className={styles.inventoryLayout}>
        <div className={styles.metricGrid}>
          <div className={styles.metricCard}>
            ...
            {isEditing ? (
              <>
                <input name="stockQuantity" type="number" min="0" step="1" className={styles.inlineInput} value={values.stockQuantity} onChange={onChange} />
                {errors.stockQuantity ? <span className={styles.inlineError}>{errors.stockQuantity}</span> : null}
              </>
            ) : (
