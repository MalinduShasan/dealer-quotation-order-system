...
router.get("/quotations", protect, dealerOnly, async (req, res) => {
  try {
    const quotations = await Quotation.find({ status: "pending" }).populate("customer", "name email").populate("items.product", "name price");
    res.json(quotations);
  } catch (error) {
